...
export interface ManagedPizza { id: number; cislo: string; nazev: string; prices: Record<PizzaSizeCode, number>; favorite?: boolean; }
const legacySideNames: Record<string, string> = {
  "\uD83C\uDF55 Slice": "Slice",
  "\uD83E\uDD56 Garlic Bread 3 ks": "Garlic Bread 3 ks",
  "\uD83E\uDD56 Garlic Bread 7 ks": "Garlic Bread 7 ks",
  "\uD83C\uDF36\uFE0F Chilli Mayo": "Chilli Mayo",
  "\uD83E\uDDC4 Garlic Mayo": "Garlic Mayo",
  "\uD83C\uDF45 Ketchup": "Ketchup",
};
export const defaults = (): ManagedPizza[] => menu.filter((item) => item.kategorie === "Pizza").map((item) => ({ id:item.id,cislo:item.cislo!,nazev:item.nazev,prices:pizzaPrices[item.pizzaPricing!],favorite:item.favorite}));
...
export const applyPizzas=(pizzas:ManagedPizza[], items:readonly MenuItem[])=>[...pizzas.map((pizza):MenuItem=>({id:pizza.id,cislo:pizza.cislo,nazev:pizza.nazev,cena:pizza.prices.S,kategorie:"Pizza",pizzaSizePrices:pizza.prices,favorite:pizza.favorite})),...items.filter((item)=>item.kategorie!=="Pizza")];
...
export const loadOtherMenu=(storage:Pick<Storage,"getItem">)=>{try{const value=JSON.parse(storage.getItem(CUSTOM_MENU_KEY)??"null");if(!Array.isArray(value)||!validOtherMenu(value))return defaultOtherMenu();const favorites=new Set([501,502,503,701]);const migrated=value.map((item:MenuItem)=>{const normalized=(item.kategorie as string)==="Omáčky"?{...item,id:item.id+3,kategorie:"Přílohy" as const}:item;return{...normalized,nazev:legacySideNames[normalized.nazev]??normalized.nazev,favorite:normalized.favorite??favorites.has(normalized.id)}});const categories=new Set(migrated.map(item=>item.kategorie));const defaults=defaultOtherMenu();const missingSides=defaults.filter(item=>item.kategorie==="Přílohy"&&!migrated.some(saved=>saved.kategorie==="Přílohy"&&saved.id===item.id));return[...migrated,...defaults.filter(item=>item.kategorie!=="Přílohy"&&!categories.has(item.kategorie)),...missingSides]}catch{return defaultOtherMenu();}};
