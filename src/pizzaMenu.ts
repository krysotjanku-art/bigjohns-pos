...
export const loadOtherMenu=(storage:Pick<Storage,"getItem">)=>{try{const value=JSON.parse(storage.getItem(CUSTOM_MENU_KEY)??"null");if(!Array.isArray(value)||!validOtherMenu(value))return defaultOtherMenu();const categories=new Set(value.map(item=>item.kategorie));return[...value,...defaultOtherMenu().filter(item=>!categories.has(item.kategorie))]}catch{return defaultOtherMenu();}};
