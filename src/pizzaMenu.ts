...
export const defaultOtherMenu=()=>menu.filter((item)=>item.kategorie==="Nápoje"||item.kategorie==="Káva").map((item)=>({...item}));
export const validOtherMenu=(items:MenuItem[])=>items.every((item)=>item.nazev.trim()&&Number.isFinite(item.cena)&&item.cena>=0);
export const loadOtherMenu=(storage:Pick<Storage,"getItem">)=>{try{const value=JSON.parse(storage.getItem(CUSTOM_MENU_KEY)??"null");return Array.isArray(value)&&validOtherMenu(value)?value:defaultOtherMenu();}catch{return defaultOtherMenu();}};
