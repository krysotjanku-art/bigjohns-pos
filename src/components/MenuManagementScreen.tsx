...
const categories: ["pizza" | MenuCategory, string, string][] = [
  ["pizza", "đźŤ• Pizzy", "Obnovit vĂ˝chozĂ­ pizzy"],
  ["Nápoje", "đźĄ¤ Nápoje", "Obnovit vĂ˝chozĂ­ nĂˇpoje"],
  ["Káva", "â• Káva", "Obnovit vĂ˝chozĂ­ kĂˇvu"],
  ["Dezerty", "đźŤŞ Dezerty", "Obnovit vĂ˝chozĂ­ dezerty"],
  ["Toppingy", "đź§€ Toppingy", "Obnovit vĂ˝chozĂ­ toppingy"],
  ["Krabice", "đź“¦ Krabice", "Obnovit vĂ˝chozĂ­ krabice"],
  ["Rozvoz", "đźš— Rozvoz", "Obnovit vĂ˝chozĂ­ rozvoz"],
];
...
export function MenuManagementScreen({ pizzas, otherItems, onSavePizzas, onSaveOther, onResetPizzas, onResetCategory, onBackToPos }: Props) {
  ...
  const addPizza = () => { const number = Math.max(0, ...pizzaDraft.map((item) => Number(item.cislo))) + 1; setPizzaDraft([...pizzaDraft, { id: Math.max(0, ...pizzaDraft.map((item) => item.id)) + 1, cislo: String(number).padStart(2, "0"), nazev: "NovĂˇ pizza", prices: { S: 0, M: 0, XL: 0 } }]); };
  const addItem = () => { if (pizza) return; const id = Math.max(base[category], ...current.map((item) => item.id)) + 1; setOtherDraft([...otherDraft, { id, nazev: "NovĂˇ poloĹľka", cena: 0, kategorie: category }]); };
  ...
  return <section className="menu-management-ui">
    <BackToPosButton onClick={onBackToPos}/><h1>SprĂˇva menu</h1>
    ...
    <div className="menu-management-ui__header"><h2>{selectedCategory[1]}</h2><div><button onClick={pizza ? addPizza : addItem}>+ PĹ™idat poloĹľku</button><button onClick={reset}>{selectedCategory[2]}</button><button onClick={save}>UloĹľit</button></div></div>
    {pizza ? <div className="menu-management-ui__cards">{pizzaDraft.map((item, index) => <article className="menu-management-ui__card" key={item.id}><label>ÄŚĂ­slo<input value={item.cislo} onChange={(event) => updatePizza(index, { cislo: event.target.value })} /></label><label>NĂˇzev<input value={item.nazev} onChange={(event) => updatePizza(index, { nazev: event.target.value })} /></label>{(["S", "M", "XL"] as const).map((size) => <label key={size}>{size}<input type="number" min="0" value={item.prices[size]} onChange={(event) => updatePizza(index, { prices: { ...item.prices, [size]: Number(event.target.value) } })} /></label>)}<div className="menu-management-ui__card-actions"><button onClick={() => setPizzaDraft(pizzaDraft.filter((_, itemIndex) => itemIndex !== index))}>Smazat</button></div></article>)}</div> : <div className="menu-management-ui__cards">{current.map((item) => <article className="menu-management-ui__card" key={item.id}><label>NĂˇzev<input value={item.nazev} onChange={(event) => updateItem(item.id, { nazev: event.target.value })} /></label><label>Cena<input type="number" min="0" value={item.cena} onChange={(event) => updateItem(item.id, { cena: Number(event.target.value) })} /></label><div className="menu-management-ui__card-actions"><button onClick={() => setOtherDraft(otherDraft.filter((entry) => entry.id !== item.id))}>Smazat</button></div></article>)}</div>}
  </section>;
}
