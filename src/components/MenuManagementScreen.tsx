...
export function MenuManagementScreen({ pizzas, otherItems, onSavePizzas, onSaveOther, onResetPizzas, onResetCategory, onBackToPos }: Props) {
  ...
  return <section className="menu-management-ui">
    ...
    {pizza ? <div className="menu-management-ui__cards">{pizzaDraft.map((item, index) => <article className="menu-management-ui__card" key={item.id}><label>Číslo<input value={item.cislo} onChange={(event) => updatePizza(index, { cislo: event.target.value })} /></label><label>Název<input value={item.nazev} onChange={(event) => updatePizza(index, { nazev: event.target.value })} /></label>{(["S", "M", "XL"] as const).map((size) => <label key={size}>{size}<input type="number" min="0" value={item.prices[size]} onChange={(event) => updatePizza(index, { prices: { ...item.prices, [size]: Number(event.target.value) } })} /></label>)}<label className="menu-management-ui__favorite">⭐ Oblíbené<input type="checkbox" checked={Boolean(item.favorite)} onChange={(event) => updatePizza(index, { favorite: event.target.checked })} /></label><div className="menu-management-ui__card-actions"><button onClick={() => setPizzaDraft(pizzaDraft.filter((_, itemIndex) => itemIndex !== index))}>Smazat</button></div></article>)}</div> : <div className="menu-management-ui__cards">{current.map((item) => <article className="menu-management-ui__card" key={item.id}><label>Název<input value={item.nazev} onChange={(event) => updateItem(item.id, { nazev: event.target.value })} /></label><label>Cena<input type="number" min="0" value={item.cena} onChange={(event) => updateItem(item.id, { cena: Number(event.target.value) })} /></label><label className="menu-management-ui__favorite">⭐ Oblíbené<input type="checkbox" checked={Boolean(item.favorite)} onChange={(event) => updateItem(item.id, { favorite: event.target.checked })} /></label><div className="menu-management-ui__card-actions"><button onClick={() => setOtherDraft(otherDraft.filter((entry) => entry.id !== item.id))}>Smazat</button></div></article>)}</div>}
  </section>;
}
