...
function App() {
  ...
  const [view, setView] = useState<"pos" | "history" | "summary" | "overview" | "backup" | "settings">("pos");
  ...
  const configuredMenu = useMemo(() => menuWithSettings(settings), [settings]);
  ...
  const handleSize = (pizza: MenuItem, size: PizzaSize) => { if (!pizza.pizzaPricing) return; addItem({ ...pizza, nazev: `${pizza.cislo} ${pizza.nazev} ${size.code}`, cena: pizzaPrices[pizza.pizzaPricing][size.code], selectedSize: size.code, vatRate: settings.reducedVat }); setSelectedPizza(null); };
  const handlePay = () => {
