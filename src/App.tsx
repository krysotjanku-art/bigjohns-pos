...
function App() {
  ...
  const handlePay = () => {
    ...
    if (isNativeUsbPrintingAvailable()) {
      void printEscPosReceipt(currentOrderReceipt).then(refreshPrinterStatus).catch(async () => { await refreshPrinterStatus(); });
  ...
  const open=(next:typeof view)=>{setDrawerOpen(false);if(["settings","menu","overview","backup"].includes(next))setPendingView(next);else setView(next)};
  ...
  if (view === "settings") return <>{navigation}<main style={{ minHeight: "calc(100vh - 52px)", padding: 20, fontFamily: "Arial", background: "#f5f5f5" }}><SettingsScreen settings={settings} appearance={appearance} accent={accent} printerStatus={printer} onRefreshPrinterStatus={()=>void refreshPrinterStatus()} onPrintTest={()=>void printEscPosReceipt(testReceipt()).then(refreshPrinterStatus).catch(refreshPrinterStatus)} onAppearanceChange={(next)=>{saveAppearance(localStorage,next);setAppearance(next)}} onAccentChange={(next)=>{saveAccent(localStorage,next);setAccent(next)}} onChangePin={(current,next,confirm)=>changePin(localStorage,current,next,confirm)} onSave={(next) => { saveSettings(localStorage, next); setSettings(next); }} onBackToPos={() => setView("pos")} /></main><Receipt receipt={receipt} /></>;
  ...
  return <>{navigation}<div className="pos-app" style={{ display: "flex", height: "calc(100vh - 52px)", fontFamily: "Arial" }}><main className="pos-menu" style={{ flex: "1 1 auto", minWidth: 0, padding: 20, background: "#f5f5f5" }}><div className="pos-menu__header"><h1>Menu</h1></div><div className="pos-menu__toolbar"><div className="pos-menu__search-wrap"><svg className="pos-menu__search-icon" aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="6" /><path d="m16 16 4 4" /></svg><input className="pos-menu__search" ref={searchInput} value={search} onChange={(event) => setSearch(event.target.value)} onKeyDown={(event) => { if (event.key === "Escape") setSearch(""); }} placeholder="Hledat položku..." /></div></div><div className="pos-menu__actions-row"><div className="pos-menu__header-actions"><button className="pos-menu__history" type="button" onClick={() => setView("suspended")}><span aria-hidden="true">⏸️</span>Pozastavené ({suspendedOrders.length})</button><button className="pos-menu__history" type="button" onClick={() => setView("history")}><span aria-hidden="true">📜</span>Historie</button></div></div><div className="pos-menu__quick-row"><QuickAddPanel items={configuredMenu} onAdd={addMenuItem}/></div>{search && <p className="pos-menu__search-status">Vyhledávání ve všech kategoriích</p>}<CategoryBar activeCategory={activeCategory} onCategoryChange={setActiveCategory} /><div className="pos-menu__content"><PizzaGrid activeCategory={activeCategory} menuItems={visibleMenu} searching={Boolean(search)} onItemSelect={(item) => { if (item.kategorie === "Pizza") setSelectedPizza(item); else { addMenuItem(item); requestAnimationFrame(() => searchInput.current?.focus()); } }} /></div></main><OrderPanel items={order} subtotal={totals.subtotal} total={totals.total} discount={totals.discount} onDiscount={()=>setDiscountOpen(true)} onIncrement={(itemKey) => { setReceipt(null); setOrder((current) => current.map((item) => orderItemKey(item) === itemKey ? { ...item, pocet: item.pocet + 1 } : item)); }} onDecrement={decrementItem} onRemove={removeItem} onSuspend={suspendOrder} onPay={handlePay} /><PizzaModal pizza={selectedPizza} onClose={() => setSelectedPizza(null)} onSizeSelect={handleSize} />{discountOpen&&<DiscountModal hasDiscount={Boolean(totals.discount)} onClose={()=>setDiscountOpen(false)} onRemove={()=>{setDiscount(null);setDiscountOpen(false)}} onApply={(type,value)=>{const next=type==="percentage"?percentageDiscount(value,totals.subtotal):fixedDiscount(value,totals.subtotal);if(next){setDiscount(next);setDiscountOpen(false)}}}/>}</div><Receipt receipt={receipt} /><DailySummaryReceipt summary={summaryPrint?.summary ?? null} issuedAt={summaryPrint?.issuedAt ?? null} /></>;
}
